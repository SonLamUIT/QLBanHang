...
namespace DAO
{
    public class BaoCaoCongNoNCC_DAO: DBConnection
    {
        DataProvider dataProvider = new DataProvider();
        public string layTongConLaiTheoSoDDH(string soddh, int thang, int nam)
        {
            return dataProvider.layDuLieuString("select sum(ConLai)[Con lai] from PHIEUNHAPHANG where SoDDH_NCC = '{0}' and MONTH(NgayGiao) = {1} and YEAR(NgayGiao) = {2}", soddh, thang, nam, "Con Lai");
        }
